...
export default function LandingPagePreview({ page, activeIdx, onActivate, isPublic = false, onSubmit }) {
    const primary = page.theme?.primary_color || "#2563EB";
    return (
        <div style={{ "--brand": primary }}>
            {page.sections.map((s, i) => {
                const interactive = !isPublic && onActivate;
                const wrapClass = interactive
                    ? `cursor-pointer transition-all ${activeIdx === i ? "ring-2 ring-[#2563EB] ring-inset" : "hover:ring-1 hover:ring-[#A1A1AA] hover:ring-inset"}`
                    : "";
                return (
...
function SectionRender({ section, primary, onSubmit, isPublic }) {
    switch (section.type) {
        case "hero":
            return (
                <section className="px-8 py-20 md:py-28 text-center relative overflow-hidden" style={{ background: section.background_image ? "" : "#F8FAFC" }}>
                    ...
                    <div className="max-w-5xl mx-auto">
                        ...
                        <div className="grid md:grid-cols-3 gap-6">
                            {(section.items || []).map((it, i) => {
                                ...
                                return (
                                    <div key={i} className="text-center p-6 border border-[#E2E8F0] rounded-sm">
        ...
        case "image_text":
            return (
                <section className="px-8 py-16 md:py-20 bg-[#F8FAFC]">
                    <div className={`max-w-5xl mx-auto grid md:grid-cols-2 gap-10 items-center ${section.position === "left" ? "md:[direction:rtl]" : ""}`}>
                        ...
                        <div className="md:[direction:ltr] aspect-[4/3] bg-white border border-[#E2E8F0] rounded-sm overflow-hidden">
                            {section.image ? (
        ...
        case "testimonial":
            return (
                <section className="px-8 py-16 md:py-20 text-white" style={{ background: "#0F172A" }}>
        ...
        case "cta":
            return (
                <section className="px-8 py-20 text-center" style={{ background: primary, color: "white" }}>
                    ...
                    {section.cta_text && (
                        <a href={section.cta_link || "#form"} className="inline-flex items-center gap-2 px-6 py-3 text-sm font-semibold bg-white text-[#0F172A] rounded-sm hover:bg-[#F8FAFC]">
                            {section.cta_text} <ArrowRight size={14} weight="bold" />
                        </a>
                    )}
                </section>
            ...
            return (
                <section className="px-8 py-16 md:py-20 bg-white">
                    <div className="max-w-3xl mx-auto">
                        ...
                        <div className="divide-y divide-[#E2E8F0] border border-[#E2E8F0] rounded-sm">
                            {(section.items || []).map((it, i) => (
...
function FormSection({ section, primary, onSubmit, isPublic }) {
    ...
    return (
        <section id="form" className="px-8 py-16 md:py-20 bg-[#F8FAFC]">
            <div className="max-w-md mx-auto bg-white border border-[#E2E8F0] p-8 rounded-sm">
