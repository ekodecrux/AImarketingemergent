            ...
            <div className="px-8 py-6 grid lg:grid-cols-[300px_1fr_360px] gap-6">
                ...
                <div className="space-y-3">
                    <div className="zm-card p-3">
                        ...
                        <input type="color" value={page.theme?.primary_color || "#002EB8"} onChange={(e) => setPage({ ...page, theme: { ...page.theme, primary_color: e.target.value } })} className="w-full h-9 cursor-pointer" data-testid="page-color" />
                    </div>
                    ...
                    <div className="zm-card">
                        <div className="flex items-center justify-between p-3 border-b border-[#E4E4E7]">
                            ...
                            <button onClick={() => setShowAdd(!showAdd)} className="text-xs uppercase tracking-[0.1em] font-bold text-[#002EB8] hover:underline flex items-center gap-1" data-testid="add-section">
                                <Plus size={11} weight="bold" /> Add
                            </button>
                        </div>
                        {showAdd && (
                            <div className="p-3 border-b border-[#E4E4E7] bg-[#F4F4F5] grid grid-cols-2 gap-1.5" data-testid="section-type-picker">
                                {SECTION_TYPES.map((t) => (
                                    <button key={t.id} onClick={() => addSection(t.id)} className="px-2 py-1.5 text-xs bg-white border border-[#E4E4E7] hover:border-[#002EB8] hover:text-[#002EB8] uppercase tracking-[0.05em] font-bold" data-testid={`add-${t.id}`}>
                                        {t.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div data-testid="sections-list">
                            {page.sections.map((s, i) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-2 px-3 py-2.5 text-left text-sm border-b border-[#E4E4E7] last:border-b-0 ${
                                        activeIdx === i ? "bg-[#09090B] text-white" : "bg-white hover:bg-[#F4F4F5]"
                                    }`}
                                >
                ...
                <div className="zm-card overflow-hidden">
                    <div className="bg-[#F4F4F5] px-4 py-2 border-b border-[#E4E4E7] flex items-center gap-2">
        ...
        <div className="zm-card sticky top-6">
            <div className="p-4 border-b border-[#E4E4E7] flex items-center justify-between">
                ...
                <div className="flex gap-1">
                    <button onClick={() => onMove(-1)} disabled={!canMoveUp} className="p-1.5 hover:bg-[#F4F4F5] disabled:opacity-30" data-testid="section-up">
                        <ArrowUp size={12} weight="bold" />
                    </button>
                    <button onClick={() => onMove(1)} disabled={!canMoveDown} className="p-1.5 hover:bg-[#F4F4F5] disabled:opacity-30" data-testid="section-down">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {section.items.map((it, i) => (
                                <div key={i} className="bg-[#F4F4F5] p-3 space-y-2 relative">
                            ...
                            {["name", "email", "phone", "company", "message"].map((f) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-2 py-1.5 text-xs uppercase tracking-[0.05em] font-bold border ${on ? "bg-[#002EB8] border-[#002EB8] text-white" : "bg-white border-[#E4E4E7] text-[#71717A]"}`}
